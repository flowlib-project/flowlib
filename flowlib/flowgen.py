...
def deploy_from_yaml(config):
    ...
    flow = yaml.safe_load(config.flow_yaml) # TODO: parse this to a real graph data structure

    # # Count how many processor groups
    # pge_name = flow_description['processor_group_name']
    # logging.info("identified {} top level process group".format(pge_name))

    # # create flow-specific processor group (under root processor group entity)
    # pge = b23.check_or_create_pg(root_pge, pge_name, b23.get_canvas_location(0))

    # # Create or Update Controllers
    # total_controllers = len(flow_description['controllers'])
    # logging.info('identified {} number of controllers'.format(total_controllers))

    # # for index, controller in enumerate(flow_description['controllers']):
    ...
    # # count how many processors in the flow.  Required for canvas rendering
    # total_processors = len(flow_description['processors'])
    # logging.info("identified {} processors in process group {}".format(total_processors, pge_name))

    # # create empty list for processor entities objects as they are created
    # pce_list = list()

    # # Iterate through YAML flow definition
    # for index, processor in enumerate(flow_description['processors']):

    #     # create processors iteratively
    #     logging.info('starting index {} for creating processor'.format(index))
    #     # get the processor entity object if new or exists, then add to list
    #     processor_entity = b23.check_or_create_proc(pge, processor, index, total_processors)
    #     pce_list.append(processor_entity)
    #     logging.info('processor {} added'.format(processor_entity.component.name))

    # connection_list = list()
    # for index, processor in enumerate(flow_description['processors']):
    #     connection = b23.check_or_create_connection(pce_list[index], processor)
